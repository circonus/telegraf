...
import (
	...
	circmgr "github.com/influxdata/telegraf/internal/circonus"
...
func (c *Circonus) Init() error {
	...
	c.processors = processors{metrics: make(chan []telegraf.Metric)}
...
func (p *processors) run(m []telegraf.Metric) {
	p.metrics <- m
}
