...
func (c *Circonus) metricProcessor(id int, metrics []telegraf.Metric, buf bytes.Buffer) int64 {
	...
	for _, m := range metrics {
		...
		case telegraf.Gauge, telegraf.Summary:
		...
		case telegraf.Histogram:
			numMetrics += c.buildHistogram(m)
		case telegraf.CumulativeHistogram:
...
func (c *Circonus) handleGeneric(m telegraf.Metric) int64 {
...
func (c *Circonus) buildNumerics(m telegraf.Metric) int64 {
	return c.handleGeneric(m)
}

// buildTexts constructs text metrics from a cua metric.
func (c *Circonus) buildTexts(m telegraf.Metric) int64 {
	return c.handleGeneric(m)
}

// buildCounters constructs counter metrics from a cua metric.
func (c *Circonus) buildCounters(m telegraf.Metric) int64 {
	dest := c.getMetricDestination(m)
	if dest == nil {
		c.Log.Warnf("no metric destination found for metric (%#v)", m)
		return 0
...
func (c *Circonus) buildHistogram(m telegraf.Metric) int64 {
	dest := c.getMetricDestination(m)
	if dest == nil {
		c.Log.Warnf("no metric destination found for metric (%#v)", m)
		return 0
...
func (c *Circonus) buildCumulativeHistogram(m telegraf.Metric) int64 {
	dest := c.getMetricDestination(m)
	if dest == nil {
		c.Log.Warnf("no metric destination found for metric (%#v)", m)
		return 0
...
func (c *Circonus) convertTags(m telegraf.Metric) trapmetrics.Tags { //nolint:unparam
...
func (c *Circonus) getMetricGroupTag(m telegraf.Metric) string {
...
func (c *Circonus) getMetricProjectTag(m telegraf.Metric) string {
	for _, t := range m.TagList() {
		if t.Key == "project_id" {
			return t.Value
		}
